...
var target_position = Vector3(0, 1, 0)
...
func _ready():
	...
	initial_pos = body_b.transform.origin - body_a.transform.origin

func _physics_process(delta):
	time += delta
	var delta_pos = (body_b.transform.origin - body_a.transform.origin) - initial_pos
	...
	# Update the target position every second to go up and down
		...
		#target_position = Vector3(1, -1, 0)
	#
	target_position = Vector3(0, cos(time), sin(time))
